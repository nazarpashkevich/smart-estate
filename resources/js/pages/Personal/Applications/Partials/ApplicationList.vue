...
export default defineComponent({
  ...
  methods: {
    ...
    getStatusClass(status: EstateApplicationStatus): string {
      return status === EstateApplicationStatus.Approved
        ? 'bg-green-700'
        : status === EstateApplicationStatus.Rejected
          ? 'bg-red-700'
          : 'bg-blue-700';
    },
  },
      ...
      <tbody>
        <template v-for="item in items">
          <tr class="bg-white border-b">
            <th
              class="px-6 pl-24 py-4 font-medium text-gray-900 whitespace-nowrap"
              scope="row"
            >
              {{ item.id }}
            </th>
            <td class="px-6 py-4">
              <Link
                :href="route('estate.show', item.estateItemId)"
                class="underline"
                >Apartment</Link
              >
            </td>
            ...
            <td class="px-6 py-4">
              ...
              <span v-else class="blur-sm"> 000 000 </span>
            </td>
            ...
            <td
              class="px-6 pr-24 py-4 text-right flex w-full space-around gap-4"
            >
              <template v-if="item.status === EstateApplicationStatus.New">
                <secondary-button
                  class="mx-auto"
                  v-on:click="
                    submitApplication(item.id, EstateApplicationStatus.Approved)
                  "
                >
                  Accept
                </secondary-button>
                <danger-button
                  class="mx-auto"
                  v-on:click="
                    submitApplication(item.id, EstateApplicationStatus.Rejected)
                  "
                >
                  Decline
                </danger-button>
